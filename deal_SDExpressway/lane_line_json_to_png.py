...
def main():
    ...
    parser.add_argument('--image_file', required=False, default='images_test')   # 原图image所在的文件夹
    parser.add_argument('--json_file', required=False, default='labels_lane_line_json')   # 标注文件json所在的文件夹
    parser.add_argument('--black_file', required=False, default='labels_lane_line_black')   # 不可见的掩膜png所在的文件夹
    parser.add_argument('--color_file', required=False, default='labels_lane_line_color')   # 可见的掩膜png所在的文件夹
    parser.add_argument('--fusion_file', required=False, default='labels_lane_line_fusion')   # 掩膜与原图叠加后的png所在的文件夹
    ...
    for i in range(0, len(list)):
        ...
        if extension == 'jpg':
            if os.path.isfile(json_path):
                ...
                mask.putpalette([0, 0, 0, 255, 153, 0]) #　putpalette给对象加上调色板，相当于上色
